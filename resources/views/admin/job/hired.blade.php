...
<section class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
        <div class="col-sm-6">
            <h1>Hired Shifts Management</h1>
        </div>
        <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="{{route('admin.home')}}">Home</a></li>
            <li class="breadcrumb-item active">Hired Shifts</li>
            </ol>
        </div>
        </div>
    </div><!-- /.container-fluid -->
...
<section class="content">
   <div class="card card-primary">
            <div class="card-header">
              <h3 class="card-title">Hired Shifts Listing</h3>
                    ...
                    @if (count($Jobs) > 0)
                        @foreach ($Jobs as $Job)
                            <tr data-entry-id="{{ $Job->id }}">
                                ...
                                <td>{{ date("H:i", strtotime($Job->grade_time_from) )  }}</td>
                                <td>{{ date("H:i", strtotime($Job->grade_time_to) )  }}</td>
